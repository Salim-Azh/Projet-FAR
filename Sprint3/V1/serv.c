...
void* controllerFile(int sender) {

  broadcast(-1, sender, "file");
  
  char nomFich[100], contenuFich[100000];

  int resNameFich = recv(tabSocket[sender].scli, nomFich, sizeof(nomFich), 0);

  if(resNameFich == -1){
    perror("Erreur de reception du message nom fichier du client");
    pthread_exit(NULL);
  }
  else if(resNameFich == 0){
    perror("Socket fermé du client nom fichier");
    pthread_exit(NULL);
  }
  printf("Nom du fichier reçu.\n");

  broadcast(-2, sender, nomFich);
  /*On recoit le nom du fichier et on l'envoi aux clients*/
  
  int resFich = recv(tabSocket[sender].scli, contenuFich, sizeof(contenuFich), 0);
  if(resFich == -1){
    perror("Erreur de reception du message du contenu fichier");
    pthread_exit(NULL);
  }
  else if(resFich == 0){
    perror("Socket fermé contenu fichier");
    pthread_exit(NULL);
  }
  
  printf("Contenue du fichier reçu\n");
  broadcast(0, sender, contenuFich);
  
  /*On recoit le contenue du fichier et on l'envoi aux clients*/
  printf("Contenue du fichier envoyé aux clients\n");
  pthread_exit(NULL);
}
...
*/
  while(1){
	...
	close(tabSocket[i].scli);
      ...
      pthread_create(&thread_send_file, NULL, (void*)&controllerFile, sender);
      pthread_join(thread_send_file, NULL);
    }
