...
void properSend(int i, int indexSalon, char* msg) {
...
void properRecv(int indexSock, int indexSalon, char* msg) {
  /*Fonction qui stocke le resultat de recv dans msg*/
  int res = recv(tabSalon[indexSalon].tabSocket[indexSock].scli, msg, sizeof(msg), 0);
...
void createSaloon(int indexSalon, int sender) {
  /*Fonction qui demande au client les caractéristique du salon qu'il veut créer et le créer*/

  // demande d'info au client
  char* name;
  properSend(sender, indexSalon, "Nom du salon : ");
  properRecv(sender, indexSalon, &name);

  char* nbMax;
  properSend(sender, indexSalon, "Nombre de personne du groupe : ");
  properRecv(sender, indexSalon, nbMax);

  int lg = sizeof(tabSalon) + 1;
  strcpy(tabSalon[lg].nom, name);
  tabSalon[lg].nbMaxCli = nbMax;
}

void deleteSaloon(int indexSalon, int sender) {
  /*Fonction qui demande au client quel salon il veut supprimer*/

  char* number;
  properSend(sender, indexSalon, "Numéro du salon à supprimer");
  properRecv(sender, indexSalon, number);

  int num = atoi(number);
  strcpy(tabSalon[num].nom, "");
  tabSalon[num].nbMaxCli = -1;
  // Nous n'afficheront que les salon de numéro suppérieur a 1
}

void modifySaloon(int indexSalon, int sender) {
  /*Fonction qui demande de modifier tous les paramètres d'un salon mais les envoie avant*/

  char* num;
  properSend(sender, indexSalon, "Quel salon voulez-vous modifier ?");
  properRecv(sender, indexSalon, num);
  int number = atoi(num);
  
  properSend(sender, indexSalon, tabSalon[number].nom);
  properRecv(sender, indexSalon, tabSalon[number].nom);

  properSend(sender, indexSalon, tabSalon[number].nbMaxCli);
  properRecv(sender, indexSalon, tabSalon[number].nbMaxCli);
}
...
void *controller(struct index_struct idx) {
  ...
  while(1){
    ...
    if(strcmp(msg, "fin") == 0){
      ...
      createSaloon(indexSalon, sender);
    }
    else if(strcmp(msg, "DELETE") == 0) {
      deleteSaloon(indexSalon, sender);
    }
    else if(strcmp(msg, "MODIFY") == 0) {
      modifySaloon(indexSalon, sender);
    }
